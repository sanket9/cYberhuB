...
export class MapingRoomsComponent implements OnInit {
  ...
  assignStudent() {
    ...
    // for (let index = 0; index < this.RoomClassStruc.length; index++) {
      
      
    // }
    let total_remain = this.filtered_room[0].total_no_students - total;
    if (total_remain < 0) {
      // console.log(this.examDaySelected[0].exam_room);
      let added_student = this.examDaySelected[0].exam_room.length - total_remain
      let slice_array = this.examDaySelected[0].exam_room.slice(0, added_student);
