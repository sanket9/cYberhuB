...
export class CoursesSubjectComponent implements OnInit {
  ...
  onChooseClassStream(e) {
    ...
    console.log((subs as FormArray).length);
    for (let index = 0; index < (subs as FormArray).length; index++) {
      (subs as FormArray).removeAt(index);
    };
    this.showloader = true;
    let header = new Headers();
    header.set("Content-Type", "application/json");
  ...
  addForm(values){
    ...
    values.controls.forEach((ele, i) => {
      ...
      console.log(ele.controls.sub_id.value);
      values.value[i].class_id = ele.controls.sub_id.value;
    });
